...
incometbl2 <- incometbl %>% 
                  mutate(region = tolower(state)) %>% 
                  select(-state)

income_data <- states %>% 
                  inner_join(incometbl2, by = "region")
...
plot_category_scheme1 <- function(cat, scheme = 1) {
    scheme1 <- c('#8c510a','#d8b365','#f6e8c3','#c7eae5','#5ab4ac','#01665e')
    scheme2 <- c('#b35806','#f1a340','#fee0b6','#d8daeb','#998ec3','#542788')
